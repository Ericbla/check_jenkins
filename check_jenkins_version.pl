...
GetOptions(\%args,
           ...
           'critical|c=s' => \$crit_vers) or pod2usage({'-exitval' => UNKNOWN});
...
if ($jenkins_version < $crit_vers) {
    $status_line = "CRITICAL: Jenkins version: $jenkins_version < crit: $crit_vers\n";
    $exit_code = CRITICAL;
}
if ($jenkins_version < $warn_vers) {
    $status_line = "WARNING: Jenkins version: $jenkins_version < warn: $warn_vers\n";
    $exit_code = WARNING;
}
...
if ($jenkins_version >= '1.466') {
    ...
    my $res = $ua->request($req);
    if (!$res->is_success) {
        print("UNKNOWN: can't get $url" . PLUGINS_URL . API_SUFFIX . "?depth=1 ($res->{status_line})");
        exit UNKNOWN;
    }
    my $json = new JSON;
