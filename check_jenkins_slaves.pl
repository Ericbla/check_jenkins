...
$instance_id =~ s#^http.*?://##;
$instance_id =~ tr#/ :,;#_#s;
$persitency_file = File::Spec->tmpdir() . '/' . PERSISTENCY_FILE_PREFIX . $instance_id . '.log';
...
my $computers = $content->{'computer'}; # ref to array
...
# Make $_warn and $_crit to represent the the absolute lower bundary of online slaves (regarding $warn and $crit %)
my $_warn = 0;
if ( $warn != -1 ) {
...
if (defined $old_slaves_status && scalar(keys(%{$old_slaves_status})) > 0) {
    ...
    foreach my $name (keys(%slaves_status)) {
        if (exists $old_slaves_status->{$name}) {
            ...
            if ($status != CRITICAL) {
                $status = WARNING;
                $status_line= "WARNING: New $name slave ($slaves_status{$name})";
            }
        }
    }
...
sub test_slave(\%) {
    ...
    trace(", $running_ex / $executors_count executors");
    if ( $slave->{'offline'} ) {
        trace(", offline");
        $total_offline_count++;
        ...
        if ( !$slave->{'idle'} ) {
            trace(", currently working");
        }
    }
    if ( $slave->{'temporarilyOffline'} ) {
        trace(", temporarily offline");
    }
    if ( defined( $slave->{'offlineCause'} ) ) {
        trace(", cause: ", $slave->{'offlineCause'}->{'description'});
    }
    trace("\n");
    if (   $ecrit != -1
        && $executors_count > 0
